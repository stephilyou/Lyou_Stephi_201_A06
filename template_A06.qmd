...
[https://github.com/stephilyou/Lyou_Stephi_201_A06](https://github.com/stephilyou/Lyou_Stephi_201_A06)
...
```{r}
lebron_index <- player_info[[2]]$points
lebron_name <- player_info[["LJ"]]$points
```
...
for (i in names(player_info)) {
  ...
  points <- player$points / player$games_played
  points_per_game[[i]] <- points
}
```
...
```{r}
points_per_game_rounded <- sapply(points_per_game, round, 2)
...
mat_rand_rounded <- apply(mat_rand, MARGIN = c(1, 2), FUN = round_func)
...
```{r}
avg_temp <- lapply(max_temps[3:14], mean)
print(avg_temp)
```
...
```{r}
colorado <- subset(x = max_temps, subset = state == "CO")
print(colorado)
ohio <- subset(x = max_temps, subset = state == "OH")
virginia <- subset(x = max_temps, subset = state == "VA")
```
...
```{r}
median_temp_annual <- function(row) {
  median(row[3:14], na.rm = TRUE)
}

median_annual_temps <- apply(colorado[, 3:14], 1, median_temp_annual)
median_temp_and_year <- cbind(colorado$year, median_annual_temps)
colnames(median_temp_and_year) <- c("year", "temp")
max_temp <- which.max(median_temp_and_year[,2])
min_temp <- which.min(median_temp_and_year[,2])

print(paste0("The maximum median annual temperature of ", median_temp_and_year[max_temp,2], " occurred in ", median_temp_and_year[max_temp,1], " and the minimum median annual temperature of ", median_temp_and_year[min_temp,2], " occurred in ", median_temp_and_year[min_temp,1]))
```
